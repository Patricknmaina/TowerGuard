...
# Import Earth Engine safely: on Windows the `ee` package may import
# Unix-only modules (e.g. `fcntl` via `blessings`) which will fail.
try:
...
def _init_ee() -> None:
    ...
    # If the ee package failed to import earlier, disable EE and log.
    if ee is None:  # pragma: no cover - platform-specific
        _EE_DISABLED = True
        log.warning(
            "Earth Engine Python package unavailable: %s; NDVI will be skipped.",
            getattr(globals(), "_EE_IMPORT_ERROR", "import error"),
        )
        return
...
def compute_ndvi_stats(
    ...
    try:
        _init_ee()
        if _EE_DISABLED:
            log.warning("Earth Engine disabled; skipping NDVI computation")
            return {
                "ndvi_mean": None,
                "ndvi_std": None,
