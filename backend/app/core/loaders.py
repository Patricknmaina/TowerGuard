...
def load_cfas(db: Database) -> int:
    """Load Community Forest Associations (CFAs) linked to water towers."""
    collection = db["cfas"]
    collection.drop()

    data_path = Path(__file__).parent.parent.parent / "data" / "nurseries" / "cfas_kenya_water_towers.csv"
    if not data_path.exists():
        print(f"CFA CSV not found at {data_path}")
        return 0

    tower_ids = {doc["id"] for doc in db["water_towers"].find()}
    tower_by_key = {}
    for doc in db["water_towers"].find():
        key = _normalize_tower_key(doc.get("name"))
        if key:
            tower_by_key[key] = doc["id"]

    # Map shorthand IDs from the CFA file to canonical tower IDs
    cfa_tower_aliases = {
        "mt_kenya": "mount_kenya",
        "mt_elgon": "mount_elgon",
        "mt_kulal": "mount_kulal",
    }

    def resolve_tower_id(raw: str | None) -> str | None:
        if not raw:
            return None
        normalized = raw.strip().lower().replace(" ", "_")
        normalized = cfa_tower_aliases.get(normalized, normalized)
        if normalized in tower_ids:
            return normalized
        name_key = _normalize_tower_key(normalized)
        return tower_by_key.get(name_key)

    docs = []
    with open(data_path, "r", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            tower_id = resolve_tower_id(row.get("water_tower_id"))
            doc = {
                "id": row.get("cfa_id") or str(uuid4()),
                "name": row.get("name"),
                "water_tower_id": tower_id,
