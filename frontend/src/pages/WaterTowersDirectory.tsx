...
const WaterTowersDirectory = () => {
  ...
  return (
    <div className="min-h-screen bg-warm-50 px-6 py-12 md:px-12 md:py-16">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <header className="mb-12 space-y-6 text-center md:text-left">
          <p className="text-xs uppercase tracking-[0.4em] text-soft-green-600 font-medium">Directory</p>
          <h1 className="text-4xl md:text-5xl font-semibold text-charcoal-900 leading-tight">Kenya's 18 Gazetted Water Towers</h1>
          <p className="text-lg text-charcoal-600 max-w-2xl">Explore each water tower's health, climate data, and biodiversity to understand restoration opportunities.</p>
          <div className="flex flex-wrap items-center gap-2 text-xs text-charcoal-600">
            <span className="inline-flex items-center gap-2 rounded-full border border-warm-200 bg-white px-3 py-1 shadow-sm">
          ...
          <div className="flex items-center gap-4">
            <select
              ...
              className="rounded-full bg-white border border-warm-300 text-charcoal-700 px-4 py-2 text-sm font-medium shadow-sm hover:border-soft-green-400 focus:outline-none focus:ring-2 focus:ring-soft-green-500 focus:border-transparent transition-all"
            >
            ...
            <button
              ...
              className="rounded-full bg-soft-green-600 hover:bg-soft-green-700 disabled:bg-soft-green-300 text-white px-6 py-2.5 text-sm font-semibold shadow-sm hover:shadow-md transition-all duration-200"
            >
              {enrichMutation.isPending ? `Refreshing last ${selectedDays} days…` : `Refresh data (last ${selectedDays} days)`}
            </button>
            {enrichMutation.isSuccess && (
              <span className="text-xs text-soft-green-700 font-medium">
                Updated {ndviStartStr} → {ndviEndStr}
              </span>
            )}
            {enrichMutation.isError && (
              <span className="text-xs text-rose-600 font-medium">Refresh failed</span>
            )}
          </div>
        </header>
