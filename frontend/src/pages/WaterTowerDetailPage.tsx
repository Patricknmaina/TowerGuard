...
const WaterTowerDetailPage = () => {
  ...
  return (
    <div className="min-h-screen bg-warm-50">
      ...
      <div className="bg-white border-b border-warm-200">
        <div className="max-w-7xl mx-auto px-6 py-12 md:px-12 md:py-16">
          <button
            ...
            className="text-sm text-charcoal-600 hover:text-charcoal-900 mb-6 inline-flex items-center gap-2"
          >
            ← Back to directory
          </button>
          
          <h1 className="text-4xl md:text-5xl font-semibold text-charcoal-900 mb-8">{tower.name}</h1>

          {/* KPI Row */}
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 md:gap-6">
            {/* NDVI Health Score - Large */}
            <div className="col-span-2 md:col-span-1 rounded-2xl border border-warm-200 bg-gradient-to-br from-white to-warm-50/50 p-6 shadow-sm">
              <p className="text-xs uppercase tracking-[0.3em] text-charcoal-500 font-medium mb-2">Health Score</p>
              <p className={`text-4xl font-bold ${getHealthScoreColor(healthScore)} mb-1`}>
            ...
            <div className="rounded-2xl border border-warm-200 bg-white p-4 shadow-sm">
              <p className="text-xs uppercase tracking-[0.2em] text-charcoal-500 font-medium mb-1">Temperature</p>
              <div className="flex items-center gap-2">
                <p className="text-2xl font-semibold text-charcoal-900">
                  {temperature != null ? `${temperature.toFixed(1)}°C` : "—"}
                </p>
                {getTrendIcon(temperature, temperature)} {/* Stub - would compare with previous */}
              </div>
            </div>

            {/* Rainfall Trend */}
            <div className="rounded-2xl border border-warm-200 bg-white p-4 shadow-sm">
              <p className="text-xs uppercase tracking-[0.2em] text-charcoal-500 font-medium mb-1">Rainfall</p>
              <div className="flex items-center gap-2">
                <p className="text-2xl font-semibold text-charcoal-900">
                  {rainfall != null ? `${rainfall.toFixed(0)} mm` : "—"}
                </p>
                {getTrendIcon(rainfall, rainfall)} {/* Stub - would compare with previous */}
              </div>
            </div>

            {/* Soil Fertility/pH */}
            <div className="rounded-2xl border border-warm-200 bg-white p-4 shadow-sm">
              <p className="text-xs uppercase tracking-[0.2em] text-charcoal-500 font-medium mb-1">Soil pH</p>
              <p className="text-2xl font-semibold text-charcoal-900">
            ...
            {/* Biodiversity */}
            <div className="rounded-2xl border border-warm-200 bg-white p-4 shadow-sm">
              <p className="text-xs uppercase tracking-[0.2em] text-charcoal-500 font-medium mb-1">Biodiversity</p>
              <p className="text-2xl font-semibold text-charcoal-900">{biodiversity.length}</p>
      ...
      <div className="max-w-7xl mx-auto px-6 py-12 md:px-12">
        <h2 className="text-3xl font-semibold text-charcoal-900 mb-6">Indigenous Species Thriving Here</h2>
        {biodiversityLoading ? (
          <DataState state="loading" />
        ) : biodiversity.length === 0 ? (
          <div className="rounded-2xl border border-warm-200 bg-white p-8 text-center">
            <p className="text-charcoal-600">No biodiversity records available for this tower yet.</p>
          </div>
        ) : (
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {biodiversity.map((species) => (
              <div
                ...
                className="rounded-2xl border border-warm-200 bg-white p-6 shadow-sm hover:shadow-md transition-shadow"
              >
      ...
      <div className="max-w-7xl mx-auto px-6 py-12 md:px-12 pb-20">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3 mb-2">
          <h2 className="text-3xl font-semibold text-charcoal-900">Recommended Species to Plant Next</h2>
          <p className="text-sm text-charcoal-500">Curated from NDVI, climate, soil, and biodiversity signals</p>
        </div>
        <p className="text-sm text-charcoal-600 mb-4">Rooted for {tower.name} so you can plant with confidence.</p>

        <div className="rounded-3xl border border-warm-200 bg-white/95 shadow-sm p-5 mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
          ...
          <button
            ...
            className="inline-flex items-center justify-center rounded-full bg-white border border-soft-green-200 px-5 py-3 text-sm font-semibold text-charcoal-800 shadow-sm hover:-translate-y-0.5 hover:shadow-md hover:bg-warm-50 transition-all duration-200"
          >
        ...
        {recommendedSpecies.length === 0 ? (
          <div className="rounded-3xl border border-warm-200 bg-white p-10 text-center shadow-sm">
            <p className="text-charcoal-700 mb-2">All common indigenous species are already present.</p>
            <p className="text-sm text-charcoal-500">Check again after the next monitoring cycle for fresh recommendations.</p>
          </div>
        ) : (
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {recommendedSpecies.map((species) => (
              <div
                ...
                className="rounded-3xl border border-warm-200 bg-white p-6 shadow-sm hover:shadow-lg transition-all duration-300"
              >
